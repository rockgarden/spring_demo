...
Now that you can see the format of data inputs and outputs, you can write code to represent a row of data, as from
...
to uppercase. See `src/main/java/com/example/batchprocessing/PersonItemProcessor.java`.
...
Now add the following beans to your `BatchConfiguration` class to define a reader, a processor, and a writer: BatchConfiguration.java.
...
shows the actual job configuration: BatchConfiguration.java[tag=jobstep]
...
such a class: JobCompletionNotificationListener.java[].
