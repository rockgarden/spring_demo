...
Introducing https://spring.io/projects/spring-hateoas[Spring HATEOAS - Hypermedia As The Engine Of Application State], a Spring project aimed at helping you write hypermedia-driven outputs. To upgrade your service to being RESTful, add this to your build:
...
[source,java,tabsize=2,indent=0]
----
// rest/src/main/java/payroll/EmployeeController.java[tag=get-single-item]
@GetMapping("/employees/{id}")
...
This simple interface has one method: `toModel()`. It is based on converting a non-resource object (`Employee`) into a resource-based object (`EntityModel<Employee>`).
